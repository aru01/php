...
*/
$isFormPosted = $_SERVER["REQUEST_METHOD"]===POST; // > Заменяем на проверку метода
...
if ($isFormPosted) {
 ...
 $validationScheme = [
   ...
   */
  $form = $_POST; // > Данные формы из запроса
...
if(count($errors)---0){
   ...
   */
  $cover = null; 
  $coverFilename = "{$clean["author"]} - {$book["title"]} ({$book["year"]}).jpg";
  var_dump($coverFilename)
